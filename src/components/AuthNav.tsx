  ...
  if (user) {
    ...
    return <details className="group relative">
      ...
      <div className={`absolute z-50 mt-2 w-72 overflow-hidden rounded-xl border border-gray-800 bg-gray-950 shadow-2xl ${compact ? "bottom-full left-0 mb-2 mt-0" : "right-0"}`}>
        <div className="border-b border-gray-800 px-3 py-3"><p className="truncate text-sm font-semibold text-gray-100">{user.email}</p><p className={`mt-0.5 text-[10px] font-semibold uppercase tracking-[0.16em] ${access?.productAccess || owner ? "text-gray-500" : "text-amber-300"}`}>{accessLabel}</p></div>
        <div className="p-1.5">
