...
const TABLE_MIGRATION = `CREATE TABLE IF NOT EXISTS radar_signals (
...
const INDEX_MIGRATION = `CREATE INDEX IF NOT EXISTS radar_signals_user_updated_idx ON radar_signals(user_id, updated_at DESC)`;
...
export async function putRadarSignal(userId: number, signal: RadarSignal): Promise<void> {
  await ensureRadarSchema();
  await sql()`insert into radar_signals (user_id, id, signal, created_at, updated_at)
