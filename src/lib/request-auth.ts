import { SESSION_COOKIE, findUserByToken, parseCookies, type AuthUser } from "./auth";
import { getAccountAccess, isPlatformOwner, type AccountAccess } from "./access.server";
...
export async function requireProductAccess(request: Request): Promise<{ user: AuthUser; access: AccountAccess }> {
...
export async function requireRequestOwner(request: Request): Promise<AuthUser> {
  const user = await requireRequestUser(request);
  if (!(await isPlatformOwner(user))) throw new Error("owner_required");
  return user;
}
