...
function OwnerDashboard() {
  ...
  useEffect(() => { if (user && owner) void refresh(); }, [user?.id, owner]);
  ...
  if (!owner) return <section className="silhat-panel mx-auto max-w-xl p-8 text-center"><h1 className="text-2xl font-bold">Owner dashboard unavailable</h1><Link to="/dashboard" className="silhat-btn silhat-btn-ghost mt-5 inline-flex">Return to Today</Link></section>;

  const invite = async () => {
    setMessage(""); setInviteUrl("");
    const response = await fetch("/api/owner/invite", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email, accessDays, inviteDays }) });
    const data = (await response.json().catch(() => ({}))) as { invite?: { token: string; email: string }; error?: string };
    if (!response.ok || !data.invite) {
      setMessage(data.error === "beta_invites_disabled" ? "Invitations are staged but disabled. Set AILHAT_BETA_INVITES_ENABLED=true when you are ready to admit the cohort." : data.error ?? "Invite could not be created.");
      return;
    }
    const origin = typeof window === "undefined" ? "" : window.location.origin;
    setInviteUrl(`${origin}/beta?invite=${encodeURIComponent(data.invite.token)}&email=${encodeURIComponent(data.invite.email)}`);
    setEmail(""); setMessage("Invite created. The token is shown once here; send it only to the invited builder.");
    await refresh();
  };
