...
// Plain REST router for all /api/* routes (availability, scan, intent, owner
// auth + portfolio). Shared with the Vercel deployment entry (vercel-entry.ts)
// so both surfaces behave identically. Bypasses TanStack's RPC machinery.
import { handleRestRoute } from "./src/lib/restRoutes.ts";

// Convenience: send JSON with a status (kept here for the static/SSR fallthrough
// if a future route needs it; the REST router has its own helpers).
// Build the session cookie config note: the `secure` flag is derived per-request
// inside handleRestRoute, so the published https URL sets Secure cookies while
// plain-http localhost (dev) does not.
...
for (let attempt = 1; ; attempt++) {
  ...
  try {
    Bun.serve({
      ...
      hostname: HOST,
      async fetch(req) {
        // The `secure` flag for cookies is derived per-request so the published
        // https URL sets Secure cookies while plain-http localhost (dev) does not.
        const secure =
          new URL(req.url).protocol === "https:" ||
          req.headers.get("x-forwarded-proto") === "https";

        // All /api/* routes are handled by the shared REST router. Non-API
        // pathnames fall through to static/SSR below.
        const restRes = await handleRestRoute(req, secure);
